...
impl Function for RegexpInStrFunction {
    ...
    fn eval(&self, columns: &ColumnsWithField, input_rows: usize) -> Result<ColumnRef> {
        ...
        if pat.is_const() && match_type.is_const() {
            ...
            return Ok(Arc::new(a_regexp_instr_binary_scalar(
                columns[0].column(),
                &pat_value,
                &pos,
                &occurrence,
        ...
        Ok(Arc::new(a_regexp_instr_binary(
            columns[0].column(),
            pat,
            &pos,
            &occurrence,
...
fn a_regexp_instr_binary_scalar(
    ...
    let re = build_regexp_from_pattern(pat, Some(mt))?;

    let iter = izip!(source, pos, occur, ro);
    for (s_value, pos_value, occur_value, ro_value) in iter {
        if pos_value < 1 {
            return Err(ErrorCode::BadArguments(format!(
                "Incorrect arguments to REGEXP_INSTR: position must be greater than 0, but got {}",
                pos_value
            )));
        }
        if ro_value != 0 && ro_value != 1 {
            return Err(ErrorCode::BadArguments(format!(
                "Incorrect arguments to REGEXP_INSTR: return_option must be 1 or 0, but got {}",
                ro_value
            )));
        }
...
fn a_regexp_instr_binary(
    ...
    for (s_value, pat_value, pos_value, occur_value, ro_value, mt_value) in iter {
        ...
        let re = if let Some(re) = map.get(&key) {
            ...
            let re = build_regexp_from_pattern(pat_value, Some(mt_value))?;
...
fn regexp_instr(s: &[u8], re: &Regex, pos: i64, occur: i64, ro: i64) -> u64 {
    let occur = if occur < 1 { 1_i64 } else { occur };
